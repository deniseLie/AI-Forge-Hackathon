...
async def _react(client, prefix, agent, scene_t):
...
async def run_panel(agents, scenario, creative, mode="fixture"):
    """Return a list of reactions. Fixture: the agents unchanged. Live: regenerated by Kimi."""
    if mode == "fixture":
        return list(agents)
    ...
    async def one(agent):
        if agent.get("status") == "abstain":
            return agent
        async with sem:
            return await _react(client, prefix, agent, scene_t)
